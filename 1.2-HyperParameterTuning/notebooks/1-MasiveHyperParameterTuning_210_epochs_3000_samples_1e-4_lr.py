            ...
            for sigma_a in hyperparam_dict["sigma_a"]:
                for sigma_b in hyperparam_dict["sigma_b"]:
                    ...
                    if check_hyperparams_in_log(log_file, hyperparams):
                        continue
                    model_instance = AnomalyModel(AnomalyTransformer.AnomalyTransformer, dataset, n_heads=3, d_model=64, enc_in=1, enc_out=1, max_norm=None, sigma_a=sigma_a, sigma_b=sigma_b, clip_sigma=clip)
                    model_instance.train(num_epochs, initial_lr, warmup_epochs, lam, lam)
                    aux=EvalModel(model_instance,dataset,hyperparams,"../results2")
